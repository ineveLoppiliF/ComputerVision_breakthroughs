...
from support_functions import indexToEliminate, plot_inliers, distance, transform, transformXY, plot_inliersXY
...
while matchesMask != None:
    if len(good) >= MIN_MATCH_COUNT:
        ...
        M, mask = cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, 2.5)
            ...
            if np.linalg.matrix_rank(M) == 3:
                if np.count_nonzero(matchesMask) >= MIN_MATCH_CURRENT:
                    ...
                    if count/len(dst_inliers) >= INSQUARE_THRESHOLD:                    
                        ...
                        polygon = Polygon([transform(M,pts[0]), transform(M,pts[1]), transform(M,pts[2]), transform(M,pts[3])])
                        
                        print("POLYGON")
                        for i in range(4):
                            print("Vertex #"+str(i))
                            print("src->("+str(pts[i][0][0])+","+str(pts[i][0][1])+") dst->"+str(transform(M,pts[i]))+"\n")
                        
                        print("---------------------------------------\n")
                        
                        for i in range(len(index_inliers)):
                            print("Inlier #"+str(i))
                            print("src->("+str(src_pts[index_inliers[i]][0][0])+","+str(src_pts[index_inliers[i]][0][1])+") dst->("+str(dst_pts[index_inliers[i]][0][0])+","+str(dst_pts[index_inliers[i]][0][1])+") dst_trans->"+str(transform(M,src_pts[index_inliers[i]])))
                            print("DISTANCE->"+str(distance(M,src_pts[index_inliers[i]],dst_pts[index_inliers[i]])))
                            point = Point(dst_pts[index_inliers[i]][0][0], dst_pts[index_inliers[i]][0][1])                    
                            if polygon.contains(point):
                                print("inside: YES\n")
                            else:
                                print("inside: NO\n")
                        ...
                        #for i in range(h):
                            #for j in range(w):
                                ...
                                #a.append(p.x)
                                #b.append(p.y)
                                #print("src->("+str(i)+","+str(j)+") dst_trans->("+str(p.x)+","+str(p.y)+")")
                                    ...
                                    #print("inside: NO\n")
                                    #countOut=countOut+1
                        #print(str(countIn)+" points mapped IN")
                        #print(str(countOut)+" points mapped OUT")
                        #plot_inliersXY(img2, dst, polygon, a, b, 'All points mapping')
