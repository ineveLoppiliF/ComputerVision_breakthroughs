...
def self_similar_and_fingerprint_main_excel_version(test): 
  ## Set the size of the figure to show
    ...
    ## Load the template
    template_image = cv2.imread(test , cv2.IMREAD_COLOR) # template image
    ...
    self_count=0;
    for i in range(len(self_similar_list)):
        if(self_similar_list[i]!=[]):
            self_count+=1
    return self_count
