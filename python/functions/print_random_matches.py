...
def print_random_matches(test_image, test_keypoints, template_image, 
    ...
    try:
        ...
        number_to_pick_up = floor(len(index_self_similar_matches)*random_match_ratio)
        
        ## Plot matches only if the number to pick up is greater than 0
        if number_to_pick_up > 0:
            ...
            for i in range(iterations):
                ...
                ## the old list
                max_elements_to_pick_up = min(len(index_self_similar_matches),number_to_pick_up)
